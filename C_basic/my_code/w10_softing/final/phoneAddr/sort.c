...
void insertionSort(data_t lst[], const int n) {
  for (int i = 0; i < n; i++) {
    data_t next = lst[i];
    int j;
    for (j = i - 1; j >= 0 && lt(next.key, &lst[j]); j--) lst[j + 1] = lst[j];
    lst[j + 1] = next;
  }
}
...
void correctHeapNode(data_t lst[], const int n, const int i) {
  ...
  const int r = l + 1;
  int m = i;
  if (l < n && gt(lst[l].key, &lst[m])) m = l;
  if (r < n && gt(lst[r].key, &lst[m])) m = r;
  if (m != i) {
    swap(&lst[i], &lst[m]);
...
void quickSort(data_t lst[], const int n) { _quicksort(lst, 0, n - 1); }
...
void _quicksort(data_t list[], const int left, const int right) {
  ...
  if (left < right) {
    i = left;
    j = right + 1;
    keyT pivot = list[left].key;  // <=== Chọn pivot
    do {
      do i++;
      while (gt(pivot, &list[i]));
      do j--;
      while (lt(pivot, &list[j]));
...
void mergeParts_aux(data_t *a, const int l, const int m, const int r) {
  ...
  data_t *q =
      (data_t *)malloc(qn * sizeof(data_t));  // q = a.slice(m + 1, m + 1 + qn);
  memcpy(q, a + m + 1, qn * sizeof(data_t));

  for (int i = 0, j = 0; i < pn || j < qn;) {
    if ((i == pn) || (j < qn && gt(p[i].key, &q[j]))) {
      a[l + i + j] = q[j];
      j++;
    } else {
      a[l + i + j] = p[i];
...
void mergeParts_inPlace(data_t lst[], const int l, int m, const int r) {
  ...
  for (; m < r; m++) {
    t = lst[m + 1];
    for (i = m; i >= l && gt(lst[i].key, &t); i--) lst[i + 1] = lst[i];
    lst[i + 1] = t;
  }
}