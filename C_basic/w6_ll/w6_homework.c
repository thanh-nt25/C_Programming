...
int main (void)
{
    data_t student[] = {{"001", "sv1", 3},
                        ...
                        {"005", "sv5", 11}};
    // tao root (head) = null
    data_t student6= {"006", "sv6", 8};
    data_t student7= {"007", "sv7", 13};
    root_t lst = llInit();
    // gan thong tin cho danh sach vao dau dsach
    for(int i=0; i<sizeof(student)/sizeof(student[0]); i++)
        lst = llInsertHead(lst, student[i]);
    
    printf("Linked list of student\n");
    llPrint(lst);
    
    printf("ll after insert student 6 (grade: 8)\n");
    lst = llInsertStudent(lst, student6);
    llPrint(lst);

    printf("ll after insert student 7 (grade: 13)\n");
    lst = llInsertStudent(lst, student7);
    llPrint(lst);
    printf("=> The list is sorted in descending order of student's grades after insert more student\n");

    char id_find[] = "002";
    printf("\nStudent has id 002\n");
    showPtrData(llFindStudent(lst,id_find));

    char id_change[] = "002";
    printf("\nll after change grade of student 002 = 9\n");
    llPrint(llChangeGrade(lst,id_change,9));

    char id_remove[] = "001";
    printf("\nRemove student has id 001\n");
    llPrint(llDeleteStudent(lst,id_remove));
